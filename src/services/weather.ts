import type { City, CurrentWeather, ForecastResponse } from "@/types/weather";
...
export const getCurrentWeather = async (
  ...
  if (!response.ok) {
    throw new WeatherApiError("현재 날씨 정보를 가져오는데 실패했습니다", response.status);
  }
...
export const getForecast = async (
  ...
  const response = await fetch(
    `${BASE_URL}/data/3.0/onecall?lat=${lat}&lon=${lon}&units=metric&lang=kr&exclude=minutely,alerts&appid=${API_KEY}`
  );

  if (!response.ok) {
    throw new WeatherApiError("예보 정보를 가져오는데 실패했습니다", response.status);
  }
